...
void tile(Monitor *m) {
  unsigned int i, n, h, mw, my, ty, bw, r, oe = enablegaps, ie = enablegaps;
  Client *c;
  ...
  for (i = 0, my = ty = m->gappoh * oe, c = nexttiled(m->clients); c;
    ...
    if (i < m->nmaster) {
      ...
      resize(c, m->wx + m->gappov * oe, m->wy + my,
             mw - (2 * c->bw) - m->gappiv * ie, h - (2 * c->bw), bw, 0);
      ...
      // (2*c->bw), 0); resize(c, m->wx + mw, m->wy + ty, m->ww - mw -
      // (2*c->bw), h - (2*c->bw), bw, 0); if (ty + HEIGHT(c) < m->wh) 	ty
      // += HEIGHT(c);
      r = n - i;
      h = (m->wh - ty - m->gappoh * oe - m->gappih * ie * (r - 1)) / r;
      resize(c, m->wx + mw + m->gappov * oe, m->wy + ty,
             m->ww - mw - (2 * c->bw) - 2 * m->gappov * oe, h - (2 * c->bw), bw,
             0);
